...
export class SeedService {
    ...
    generateTransactionHash(fromUserId, toUserId, amount, timestamp) {
    ...
    async checkVelocityLimits(userId) {
        ...
        const todayTransfers = await db
            ...
            .executeTakeFirst();
        const transferCount = parseInt(todayTransfers?.count || '0');
        return transferCount < 3;
    }
    // Process transaction with enhanced security
    async processTransaction(fromUserId, toUserId, amount, type, description = null, dreamId = null) {
    ...
    async autoPollination(dreamId, profitAmount) {
        ...
        const eligibleDreams = nearbyDreams.filter(d => {
            const distance = this.calculateDistance(dream.location_lat, dream.location_lng, d.location_lat, d.location_lng);
        ...
        for (const targetDream of eligibleDreams) {
            ...
            const creator = await db
                ...
                .executeTakeFirst();
            if (creator) {
                await this.processTransaction(null, creator.id, amountPerDream, 'POLLINATION', `Auto-pollination from dream: ${dream.title}`, dreamId);
            }
        }
    ...
    async cleanupExpiredSeeds() {
        const sixMonthsAgo = new Date();
        sixMonthsAgo.setMonth(sixMonthsAgo.getMonth() - 6);
        // Find wallets with no activity for 6 months
        const inactiveWallets = await db
            .selectFrom('seed_wallets')
            .leftJoin('seed_transactions', (join) => join.on((eb) => eb.or([
            eb('seed_transactions.from_user_id', '=', eb.ref('seed_wallets.user_id')),
            eb('seed_transactions.to_user_id', '=', eb.ref('seed_wallets.user_id'))
        ])))
        ...
        for (const wallet of inactiveWallets) {
            ...
            await this.processTransaction(wallet.user_id, null, wallet.balance, 'SPENDING', 'Expired seeds returned to community pool', null);
