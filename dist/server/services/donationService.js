...
export class DonationService {
    ...
    async createDonationLink(type, dreamId, category, customMessage) {
        const linkId = uuidv4();
        const link = await db
    ...
    async registerDonor(email, name, preferredCurrency = 'USD') {
    ...
    calculateConversionRate(donationType) {
        switch (donationType) {
    ...
    async processDonation(donorId, amount, currency, donationType, targetDreamId, targetCategory, donorMessage, paymentReference) {
        ...
        return await db.transaction().execute(async (trx) => {
            ...
            await this.recordSeedFlowEvent(trx, 'DONATION', `donor:${donorId}`, donationType === 'SPECIFIC_PROJECT' ? `dream:${targetDreamId}` : 'seedbank', amount, seedsGenerated, targetDreamId || null, donation.donation_id);
            console.log(`Processed donation: ${amount} ${currency} → ${seedsGenerated} seeds`);
            return donation;
        });
    }
    // Distribute seeds based on donation type
    async distributeSeedsFromDonation(trx, donation) {
            ...
            case 'SPECIFIC_PROJECT':
                if (donation.target_dream_id) {
                    ...
                    const dream = await trx
                        ...
                        .executeTakeFirst();
                    if (dream) {
                        await this.seedService.processTransaction(null, dream.user_id, donation.seeds_generated, 'EARNING', `Donation: ${donation.donor_message || 'Supporting your dream'}`, donation.target_dream_id);
                    }
                }
                break;
            case 'CATEGORY_POOL':
    ...
    async distributeToCategoryProjects(trx, category, totalSeeds) {
        ...
        const categoryDreams = await trx
            ...
            .execute();
        if (categoryDreams.length === 0)
            return;
        const seedsPerDream = totalSeeds / categoryDreams.length;
        for (const dream of categoryDreams) {
            await this.seedService.processTransaction(null, dream.user_id, seedsPerDream, 'EARNING', `Category pool donation: ${category}`, dream.id);
        }
    ...
    async autoPollinateFromSeedbank(trx, totalSeeds) {
        ...
        for (const dream of activeDreams) {
            await this.seedService.processTransaction(null, dream.user_id, seedsPerDream, 'POLLINATION', 'Seedbank auto-pollination', dream.id);
        }
        console.log(`Auto-pollinated ${totalSeeds} seeds to ${activeDreams.length} projects`);
    }
    // Record seed flow event for transparency
    async recordSeedFlowEvent(trx, eventType, fromEntity, toEntity, amount, seedsInvolved, dreamId, transactionHash) {
        await trx
    ...
    async stakeSeeds(userId, dreamId, amount, monthsLocked = 12) {
        ...
        await db.transaction().execute(async (trx) => {
            ...
            await trx
                ...
                .execute();
            // Record event
            await this.recordSeedFlowEvent(trx, 'STAKE', `user:${userId}`, `dream:${dreamId}`, amount, amount, dreamId, `stake_${Date.now()}`);
        });
    ...
    async getDonationAnalytics() {
        ...
        return {
            ...
            conversion_efficiency: totalDonations?.total_amount ?
                (Number(totalDonations.total_seeds) / Number(totalDonations.total_amount) * 100).toFixed(1) + '%' : '0%'
        };
    }
