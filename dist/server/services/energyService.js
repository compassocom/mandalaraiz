...
export class EnergyService {
    ...
    async getCurrentEnergyStatus(dreamId) {
    ...
    async calculateEnergyMetrics(dreamId) {
        // Get dream info
        ...
        const taskStats = await db
            .selectFrom('tasks')
            .select([
            (eb) => eb.fn.countAll().as('total_tasks'),
            (eb) => eb.fn.count('id').filterWhere('status', '=', 'COMPLETED').as('completed_tasks'),
            (eb) => eb.fn.count('id').filterWhere('status', '=', 'IN_PROGRESS').as('in_progress_tasks')
        ])
            .where('dream_id', '=', dreamId)
            .executeTakeFirst();
    ...
    async updateEnergyMetrics(dreamId) {
        const energyData = await this.calculateEnergyMetrics(dreamId);
        await db
            .insertInto('energy_logs')
            .values({
